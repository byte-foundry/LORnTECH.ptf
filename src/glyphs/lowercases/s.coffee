		...
		0:
			...
			nodes:
				...
				3:
					...
					dirOut: Math.min(
						Math.PI,
						Utils.lineAngle(contours[0].nodes[2].point, contours[0].nodes[4].point) + ( 10 * (width-1) * 1.3 ) / 180 * Math.PI + Math.PI / 6
					)
					type: 'smooth'
					tensionIn: 1.1
					tensionOut: 1.1
				...
				4:
					...
					tensionIn: 1.1
					expand: Object({
						width: ( 85 / 80 ) * thickness + Math.max(0, (width - 1) * 40 * (thickness / 80 - 1))
						angle: 180 + 'deg'
						distr: 0.75
					})
				5:
